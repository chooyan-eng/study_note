...
| T08 | レイヤー機能の実装 | 完了 | Phase 3 |
| T09 | 計算記号図形（スタンプ）の実装 | 完了 | Phase 2 |
| T10 | 任意サイズ図形（ドラッグ描画）の実装 | 完了 | Phase 2 |
| T11 | オブジェクト選択・編集の実装 | 完了 | Phase 2 |
| T12 | スナップショットの実装 | 完了 | Phase 4 |
| T13 | 画像取り込みの実装（Swift / PHPicker） | 完了 | Phase 5 |
| T14 | 画像切り抜きの実装（crop_your_image） | 完了 | Phase 5 |
| T15 | 画像加工パイプラインの実装 | 完了 | Phase 5 |
| T16 | キャンバスへの画像貼り付け | 完了 | Phase 5 |
...
**目的:** ドラッグ操作で任意サイズの四角形・円を描画する。

**実装内容:**

- `free_shape.dart` の `FreeShape` を完成させる（`FreeShapeType`: rect / circle）
- ツール選択で「四角形」「円」を切り替え（「楕円」は不要）
- 四角形は「始点と終点から算出した4つの `StrokePoint` からなる線」として扱う。そうなるように `onStrokeUpdated` でデータを加工する
- 円は、「始点を中心とし、始点から終点までの距離を半径とした円」として扱う。そうなるように `onStrokeUpdated` でデータを加工し、かつそのデータから円を描く `pathBuilder` を自作する
- 確定時に `FreeShape` を `CanvasState.objects` に追加
- 操作を Undo スタックに積む
- ※ 描画はすべて `Draw` で行う。新しい `CustomPainter` は追加しない
...
- 選択中に表示するプロパティパネル（色 / 線種（線のみ） / 削除ボタン）
- ハンドルドラッグによるリサイズ（`Rect` の更新）
  - 線、任意サイズの図形はハンドルでリサイズ
  - スタンプは追加時と同じ UI でスライダーでリサイズ
- 各変更操作を Undo スタックに積む
...
**完了条件:**
- 加工後の画像が鮮明で色が保持されている。文字や図がくっきりとみやすく、背景や影が極力なくなっている。
- UI スレッドがブロックされない
